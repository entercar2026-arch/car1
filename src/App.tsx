...
const WhatsAppIcon: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
  <svg
    ...
    className={className}
  >
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.693.625.713.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.746.953 3.71 1.454 5.709 1.455h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
  </svg>
);
...
const PrintPreviewOverlay = React.memo(({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const pageElements = quotationRef.current.querySelectorAll(".pdf-page-print");
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-stone-950/95 backdrop-blur-md select-none no-print font-sans">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-8 flex flex-col items-center bg-stone-900/40 w-full">
        ...
        <div
          ref={quotationRef}
          className="w-full max-w-4xl transition-all duration-200 ease-out flex flex-col gap-10 items-center mt-4 print:gap-0 print:m-0 print:p-0"
        ...
        return parsedCars.map((car: Car) => {
          ...
          if (initialCar) {
            return {
              ...
              shortTermPricing: car.shortTermPricing ?? initialCar.shortTermPricing,
            };
          }
          return { ...car, isShortTermAvailable: car.isShortTermAvailable ?? true, shortTermPricing: car.shortTermPricing ?? [{ days: "1 Day", price: "$150", deposit: "$500" }, { days: "1 Week", price: "$800", deposit: "$500" }] };
        });
      } catch (e) {
          ...
          if (data.length === 0) {
            ...
            if (refreshedData) {
              setCars(refreshedData.map(car => {
                ...
                if (initialCar) {
                  return { ...initialCar, ...car, isShortTermAvailable: car.isShortTermAvailable || initialCar.isShortTermAvailable, shortTermPricing: car.shortTermPricing || initialCar.shortTermPricing };
                }
                return { ...car, isShortTermAvailable: car.isShortTermAvailable ?? true, shortTermPricing: car.shortTermPricing ?? [{ days: "1 Day", price: "$150", deposit: "$500" }, { days: "1 Week", price: "$800", deposit: "$500" }] };
              }));
            }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 select-none">
              ...
              <ul className="space-y-2.5 text-xs text-stone-400">
                <li>
                  <motion.button
                    whileHover={{ scale: 1.05, x: 4, color: "#fcd34d" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { scrollToAnchor("category-filter-container"); setFilters(prev => ({ ...prev, brand: 'Toyota' })); }}
                    className="transition-colors cursor-pointer text-left font-semibold flex items-center group hover:text-amber-300"
                  >
                ...
                <li>
                  <motion.button
                    whileHover={{ scale: 1.05, x: 4, color: "#fcd34d" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { scrollToAnchor("category-filter-container"); setFilters(prev => ({ ...prev, brand: 'Lexus' })); }}
                    className="transition-colors cursor-pointer text-left font-semibold flex items-center group hover:text-amber-300"
                  >
                ...
                <li>
                  <motion.button
                    whileHover={{ scale: 1.05, x: 4, color: "#fcd34d" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { scrollToAnchor("category-filter-container"); setFilters(prev => ({ ...prev, brand: 'Ford' })); }}
                    className="transition-colors cursor-pointer text-left font-semibold flex items-center group hover:text-amber-300"
                  >
                ...
                <li>
                  <motion.button
                    whileHover={{ scale: 1.05, x: 4, color: "#fcd34d" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { scrollToAnchor("category-filter-container"); setFilters(prev => ({ ...prev, brand: 'MG' })); }}
                    className="transition-colors cursor-pointer text-left font-semibold flex items-center group hover:text-amber-300"
                  >
                ...
                <li>
                  <motion.button
                    whileHover={{ scale: 1.05, x: 4, color: "#fcd34d" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { scrollToAnchor("category-filter-container"); setFilters(prev => ({ ...prev, brand: 'Nissan' })); }}
                    className="transition-colors cursor-pointer text-left font-semibold flex items-center group hover:text-amber-300"
                  >
                ...
                <li>
                  <motion.button
                    whileHover={{ scale: 1.05, x: 4, color: "#fcd34d" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { scrollToAnchor("category-filter-container"); setFilters(prev => ({ ...prev, brand: 'Mazda' })); }}
                    className="transition-colors cursor-pointer text-left font-semibold flex items-center group hover:text-amber-300"
                  >
              ...
              <ul className="space-y-4 text-xs text-stone-400">
                <li>
                  <motion.a
                    ...
                    whileTap={{ scale: 0.95 }}
                    href="#"
                    className="transition-colors flex items-center group hover:text-amber-300"
                  >
          ...
          <div className="mt-12 pt-8 border-t border-stone-800 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-stone-500 font-sans">
            ...
            <div className="flex gap-4">
              <motion.span
                whileHover={{ scale: 1.05, color: "#d6d3d1" }}
                whileTap={{ scale: 0.95 }}
