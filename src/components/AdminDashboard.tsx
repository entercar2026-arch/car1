...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Tabs management
  const [activeTab, setActiveTab] = useState<'fleet' | 'bookings' | 'reviews'>('fleet');

  // Modal states
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingCar, setEditingCar] = useState<Car | null>(null);
  const [carToDelete, setCarToDelete] = useState<string | null>(null);
  const [activePassportUrl, setActivePassportUrl] = useState<string | null>(null);
  ...
  const [formTransmission, setFormTransmission] = useState<Car['transmission']>('Automatic');
  ...
  const SUGGESTED_IMAGES = [
    { name: 'Porsche 911 Coupe', url: 'https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?auto=format&fit=crop&q=80&w=600' },
    { name: 'Mercedes G-Wagon', url: 'https://images.unsplash.com/photo-1520050206274-a1ae446cb3cc?auto=format&fit=crop&q=80&w=600' },
    { name: 'Tesla Model S Custom', url: 'https://images.unsplash.com/photo-1617788138017-80ad40651399?auto=format&fit=crop&q=80&w=600' },
    { name: 'Range Rover Sport', url: 'https://images.unsplash.com/photo-1508974239320-0a029497e820?auto=format&fit=crop&q=80&w=600' }
  ];
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const targetImageUrl = formImage.trim() !== '' ? formImage : 'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&q=80&w=600';
  ...
  const avgRate = totalAssets > 0 ? Math.round(cars.reduce((sum, c) => sum + c.price, 0) / totalAssets) : 0;
  const activeBookingsCount = bookings.filter(b => b.status === 'Approved').length;

  return (
    <div id="admin-dashboard-root" className="min-h-screen bg-[#FAFAF9]" style={{ color: '#1C1917' }}>
      
      {/* Top Professional Admin Bar */}
      <header id="admin-top-bar" className="bg-white border-b border-stone-200 sticky top-0 z-30 shadow-sm col-span-3">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        <section id="admin-metrics" className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
          <div className="bg-white p-5 rounded-3xl border border-stone-100 shadow-xs flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Fleet Strength</p>
              <h3 className="text-2xl font-black text-stone-850 mt-0.5">{totalAssets} Vehicles</h3>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-3xl border border-stone-100 shadow-xs flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Average Rent Fee</p>
              <h3 className="text-2xl font-black text-stone-850 mt-0.5">${avgRate} /month</h3>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-3xl border border-stone-100 shadow-xs flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Approved Bookings</p>
              <h3 className="text-2xl font-black text-stone-850 mt-0.5">{activeBookingsCount} active</h3>
            </div>
          </div>
        </section>
        ...
        <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden mb-12">
          ...
          {activeTab === 'fleet' && (
            <div id="fleet-inventory-panel">
              ...
              <div className="p-6 border-b border-stone-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="font-sans font-black text-xl text-stone-900 tracking-tight">Active Inventory Manager</h2>
                  <p className="text-xs text-stone-500 mt-1 leading-normal">
                    Directly handle additions, revisions, and description updates. Updates broadcast instantly.
                  </p>
                </div>
              ...
              {cars.length === 0 ? (
                <div className="p-12 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="font-semibold text-stone-800 text-sm">No cars available</p>
                  <p className="text-xs text-stone-400 max-w-sm mt-1 mb-4 leading-normal">
                    Your database is empty. Click "Add New Vehicle" to seed listings.
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-stone-100">
                      {cars.map((car) => (
                        <tr id={`admin-row-${car.id}`} key={car.id} className="hover:bg-stone-50/40 transition-colors">
                          ...
                          <td className="p-4">
                            <span id={`admin-name-${car.id}`} className="font-bold text-stone-900 block text-sm">
                              {car.name}
                            </span>
                            <span className="text-[10px] text-stone-400 line-clamp-1 max-w-xs leading-none mt-1">
                              {car.description || 'Premium curated vehicle asset'}
                            </span>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4 font-mono">
                            <span id={`admin-fee-${car.id}`} className="font-extrabold text-stone-900 text-sm">
                              ${car.price}
                            </span>
                          ...
                          <td className="p-4">
                            <div className="flex gap-2 text-[10px] font-mono text-stone-500 font-semibold uppercase">
                              ...
                              <span>•</span>
                              <span className="text-amber-700 font-bold">{car.yearModel || '2024'}</span>
                            </div>
                          </td>
          ...
          {activeTab === 'bookings' && (
            <div id="bookings-registry-panel">
              <div className="p-6 border-b border-stone-100">
                <h2 className="font-sans font-black text-xl text-stone-900 tracking-tight">Active Bookings Registry</h2>
                <p className="text-xs text-stone-500 mt-1 leading-normal animate-fade-in">
                  Authenticate pickup vouchers, track current dispatch states, and cancel unconfirmed requests.
                </p>
              </div>
              ...
              {bookings.length === 0 ? (
                <div className="p-16 text-center select-none">
                  ...
                  <p className="font-bold text-stone-700 text-sm">No Active Booking Records</p>
                  <p className="text-xs text-stone-400 mt-1 max-w-sm mx-auto leading-normal">
                    When customers submit bookings in the catalog section, details of pickup dates, and totals materialize here.
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-stone-50 text-[10px] font-bold tracking-wider text-stone-400 uppercase border-b border-stone-150 select-none">
                        ...
                        <th className="p-4">Pickup / Return Dates</th>
                    ...
                    <tbody className="divide-y divide-stone-100 text-xs">
                      {bookings.map((book) => (
                        <tr key={book.id} className="hover:bg-stone-50/40 transition-colors">
                          ...
                          <td className="p-4">
                            <p className="font-bold text-stone-950">{book.customerName}</p>
                            <p className="text-[10px] text-stone-500 font-mono select-all font-medium mt-0.5">{book.customerEmail}</p>
                            <div className="flex flex-col gap-1.5 mt-1">
                              <span className="text-[10px] text-stone-405 font-mono font-medium">{book.customerPhone}</span>
                              {book.passportPhoto ? (
                                <div>
                                  <button
                                    id={`btn-view-passport-${book.id}`}
                                    onClick={() => setActivePassportUrl(book.passportPhoto || null)}
                                    className="inline-flex items-center gap-1 bg-amber-100 hover:bg-amber-110 text-amber-900 border border-amber-200 text-[9px] font-extrabold px-2 py-0.5 rounded-md cursor-pointer transition-all shadow-5xs"
                                  >
                                ...
                                <div className="text-[9px] text-stone-400 font-bold flex items-center gap-1">
                                  <FileWarning className="w-3 h-3 text-stone-400" />
                                  <span>NO PASSPORT BACKLOGGED</span>
                                </div>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="p-4">
                            <div className="flex flex-col font-mono text-stone-700 font-semibold">
                              <span>Pickup: {book.pickupDate}</span>
                              <span className="text-stone-400 mt-0.5">Return: {book.returnDate}</span>
                            </div>
                          </td>
                          ...
                          <td className="p-4">
                            <span className={`px-2.5 py-1 rounded-xl text-[10px] font-bold tracking-normal uppercase ${
                              book.status === 'Approved' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                              book.status === 'Cancelled' ? 'bg-rose-50 text-rose-800 border border-rose-100' :
                          ...
                          <td className="p-4 text-right pr-6">
                            <div className="flex items-center justify-end gap-1.5">
                              {book.status === 'Pending' && onUpdateBookingStatus && (
                                <button
                              ...
                              {book.status !== 'Cancelled' && onUpdateBookingStatus && (
                                <button
          ...
          {activeTab === 'reviews' && (
            <div id="reviews-moderation-panel">
              <div className="p-6 border-b border-stone-100">
                <h2 className="font-sans font-black text-xl text-stone-900 tracking-tight">Reviews Moderation Center</h2>
                <p className="text-xs text-stone-500 mt-1 leading-normal animate-fade-in">
                  Approve and publish customer feedback stories to display dynamically inside the product catalog card lists.
                </p>
              </div>
              ...
              {reviews.length === 0 ? (
                <div className="p-16 text-center select-none">
                  ...
                  <p className="font-bold text-stone-700 text-sm">No Customer Reviews Logged</p>
                  <p className="text-xs text-stone-400 mt-1 max-w-sm mx-auto leading-normal">
                    Whenever catalog visitors write thoughts and input ratings, they undergo review cycles and list under this dashboard.
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-stone-50 text-[10px] font-bold tracking-wider text-stone-400 uppercase border-b border-stone-150 select-none">
                        ...
                        <th className="p-4 text-right pr-6 w-52">Moderator Action Controls</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100 text-xs">
                      {reviews.map((rev) => {
                        ...
                        return (
                          <tr key={rev.id} className="hover:bg-stone-50/40 transition-colors">
                            <td className="p-4 pl-6">
                              <p className="font-bold text-stone-900">{rev.customerName}</p>
                              {targetCar && (
                                <span className="text-[10px] text-[#4C0027] font-semibold block mt-0.5">{targetCar.name}</span>
                              )}
                            </td>
                            <td className="p-4">
                            ...
                            <td className="p-4">
                              <span className={`px-2 py-0.5 rounded-lg text-[9px] font-bold tracking-widest uppercase ${
                                rev.isApproved ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'
                              }`}>
      ...
      <AnimatePresence>
        {isFormOpen && (
          <div id="modal-upsert" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs overflow-y-auto">
            <motion.div
              ...
              <div className="bg-[#4C0027] text-white p-5 flex items-center justify-between border-b border-[#5E0030]/50">
                <div>
                  <h3 className="font-sans font-extrabold text-base">
                    {editingCar ? 'Update Fleet Record' : 'Register New Vehicle'}
                  </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      value={formCategory}
                      onChange={(e) => setFormCategory(e.target.value as Car['category'])}
                      className="w-full px-3.5 py-2 border border-stone-200 bg-stone-50 rounded-xl text-stone-850 text-xs focus:bg-white focus:outline-none focus:border-[#4C0027] transition-all"
                    >
                  ...
                  <div>
                    ...
                    <select
                      id="select-car-fuel"
                      value={formFuel}
                      onChange={(e) => setFormFuel(e.target.value as Car['fuelType'])}
                      className="w-full px-3.5 py-2 border border-stone-200 bg-stone-50 rounded-xl text-stone-850 text-xs focus:bg-white focus:outline-none focus:border-[#4C0027] transition-all"
                    >
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div className="mt-2">
                      <span className="text-[9px] text-stone-400 font-sans block mb-1">Choose a suggested preset image placeholder:</span>
      ...
      <AnimatePresence>
        {carToDelete !== null && (
          <div id="modal-delete-confirm" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
            ...
            >
              <div className="flex flex-col items-center justify-center text-center">
                ...
                <h3 className="font-bold text-stone-900 text-base">Confirm Fleet Removal</h3>
                
                <p className="text-xs text-stone-500 mt-2 px-1 leading-relaxed">
                  Are you absolutely sure you want to delete this vehicle listing? This action cannot be undone, and the deletion immediately purges client screens.
                </p>
      ...
      <AnimatePresence>
        {activePassportUrl !== null && (
          <div id="modal-passport-preview" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/65 backdrop-blur-xs" onClick={() => setActivePassportUrl(null)}>
            <motion.div
            ...
            >
              ...
              <div className="w-full text-center mb-4">
                ...
                <h3 className="font-extrabold text-stone-900 text-sm">Driver Verification Document</h3>
                <p className="text-[11px] text-stone-400 mt-1 leading-normal">
                  Scanned passport verification proof submitted during online vehicle rental booking
                </p>
              </div>

              <div className="w-full bg-stone-50 rounded-2xl border border-stone-200 overflow-hidden flex items-center justify-center p-2 relative" style={{ minHeight: '200px' }}>
                <img src={activePassportUrl} className="max-h-[350px] max-w-full rounded-lg object-contain shadow-sm" alt="Driver Passport View" />
              </div>
